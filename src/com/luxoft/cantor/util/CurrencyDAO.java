...
public class CurrencyDAO {
	public void persist(String code, String name, String salary) {
		EntityManager em = HibernateUtil.getEntityManager();
		EntityTransaction transaction = em.getTransaction();
		Dibs dibs = new Dibs.BuildDibs().addCode(code).addName(name).addRate(salary).build();
		try {
			transaction.begin();
			em.persist(dibs);
			transaction.commit();
	...
	public List<Dibs> findAll() {
		...
		try {
			list = (List<Dibs>) em.createQuery("from Dibs", Dibs.class).getResultList();
		} catch (Exception e) {
